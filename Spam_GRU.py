...
def rnn_model(features):
    embedding = tf.Variable(tf.random_uniform([3879, 300], -1.0, 1.0), trainable=True)
    embed = tf.nn.embedding_lookup(embedding, features)
    cell = tf.nn.rnn_cell.GRUCell(2)
    rnn_out, state = tf.nn.dynamic_rnn(cell, embed, dtype=tf.float32)
    last_output = rnn_out[:, -1]
    dense = tf.layers.dense(inputs=last_output, units=2, activation=tf.nn.relu)

    return dense

def main():
    # Load dataset
    train_data, test_data = load_text_dataset(path='./spam/', seq_len=5)
    ...
    sess.run(init)
    save_path = saver.save(sess, "spam_gru_model/model.ckpt")
    ...
    plot_confusion_matrix(all_predictions, test_y, title="RnnNet")
    plt.show()
